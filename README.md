# <center>Databases<center>

## Some materials and examples of databases that are being explored and applied.  
1. [MySQL](https://github.com/mtemporim/Databases/tree/main/MySQL)
1. [Oracle](https://github.com/mtemporim/Databases/tree/main/Oracle)



## Some links to the best databases to many functionalities  

1.  [MySQL](https://www.mysql.com/)
1.  [SQL Server](https://www.microsoft.com/en-us/sql-server/sql-server-2019)  
1.  [Oracle](https://www.oracle.com/es/database/technologies/)
1.  [PostgreSQL](https://www.postgresql.org/)
1.  [MariaDB](https://mariadb.org/)
1.  [SQLite](https://sqlite.org/index.html)
1.  [FirebirdSQL](https://firebirdsql.org/)
1.  [AuroraDB](https://aws.amazon.com/es/rds/aurora/)
1.  [Db2](https://www.ibm.com/products/db2)
1.  [MongoDB](https://www.mongodb.com/)
1.  [Redis](https://redis.io/)
1.  [Cassandra](https://cassandra.apache.org/)
1.  [InfluxDB](https://www.influxdata.com/)
1.  [DynamoDB](https://aws.amazon.com/es/dynamodb/)
1.  [Solr](https://solr.apache.org/)
1.  [ElasticSearch](https://www.elastic.co/)
1.  [Prometheus](https://prometheus.io/)







